"""
Models Module
=============
Implementasi CNN-LSTM hybrid architecture untuk prediksi koordinat GNSS.
"""

import logging
import numpy as np
from typing import Tuple, Optional
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
from dataclasses import dataclass

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


@dataclass
class ModelConfig:
    """Configuration untuk CNN-LSTM model."""
    # Input/Output
    input_length: int = 30           # Lookback window
    output_length: int = 7           # Prediction horizon
    n_features: int = 3              # E, N, U
    
    # CNN parameters
    cnn_enabled: bool = True
    cnn_kernels: Tuple = (3, 5, 7)
    cnn_filters: Tuple = (32, 64, 128)
    cnn_dropout: float = 0.3
    
    # LSTM parameters
    lstm_hidden: int = 64
    lstm_layers: int = 2
    lstm_dropout: float = 0.2
    lstm_bidirectional: bool = True
    
    # Dense layers
    dense_units: Tuple = (128, 64)
    dense_activation: str = "relu"
    
    # Training
    learning_rate: float = 0.001
    weight_decay: float = 1e-5
    
    # Device
    device: str = "cuda" if torch.cuda.is_available() else "cpu"


class GNSSDataset(Dataset):
    """Dataset untuk GNSS time series prediction."""
    
    def __init__(self, data: np.ndarray, window_size: int, horizon: int):
        """
        Initialize dataset.
        
        Args:
            data (np.ndarray): Input data (n_samples, n_features)
            window_size (int): Lookback window
            horizon (int): Prediction horizon
        """
        self.data = torch.FloatTensor(data)
        self.window_size = window_size
        self.horizon = horizon
        self.n_samples = len(data) - window_size - horizon + 1
    
    def __len__(self) -> int:
        return max(0, self.n_samples)
    
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get sample pair (input, target)."""
        x = self.data[idx:idx + self.window_size]
        y = self.data[idx + self.window_size:idx + self.window_size + self.horizon]
        return x, y


class CNNLayer(nn.Module):
    """1D CNN layer dengan multiple kernel sizes."""
    
    def __init__(self, in_channels: int, out_channels: int, 
                 kernel_sizes: Tuple = (3, 5, 7), dropout: float = 0.3):
        """
        Args:
            in_channels (int): Input channels
            out_channels (int): Output channels
            kernel_sizes (Tuple): Multiple kernel sizes
            dropout (float): Dropout rate
        """
        super().__init__()
        
        self.convs = nn.ModuleList([
            nn.Conv1d(in_channels, out_channels, kernel_size=k, padding=k//2)
            for k in kernel_sizes
        ])
        
        self.dropout = nn.Dropout(dropout)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool1d(2)
    
    # def forward(self, x: torch.Tensor) -> torch.Tensor:
    #     """
    #     Forward pass.
        
    #     Args:
    #         x (torch.Tensor): (batch, channels, length)
        
    #     Returns:
    #         torch.Tensor: (batch, n_kernels*channels, length//2)
    #     """
    #     conv_outs = [self.relu(conv(x)) for conv in self.convs]
    #     concat = torch.cat(conv_outs, dim=1)
    #     concat = self.dropout(concat)
    #     out = self.pool(concat)
    #     return out
    
    def forward(self, x):
        # x memiliki shape awal: [Batch_size, Sequence_length, N_features] -> [32, Seq_len, 300]
        
        # 1. Adaptasi untuk CNN (CNN 1D biasanya menerima [Batch, Features, Sequence])
        # Jika tensor Anda masih [32, Seq_len, 300], lakukan transpose ke [32, 300, Seq_len]
        if x.shape[-1] == self.config.n_features:
            x = x.transpose(1, 2)
            
        # 2. Lewatkan ke Blok CNN
        x_cnn = self.cnn_block(x) 
        # Misal keluaran x_cnn adalah: [Batch_size, New_Channels, New_Sequence_Length]
        
        # 3. Solusi RuntimeError Reshape:
        # Kembalikan dimensi ke bentuk sekuensial untuk LSTM: [Batch_size, New_Sequence_Length, New_Channels]
        # Kita gunakan permute, bukan menjebak fitur kembali ke 300 secara paksa
        x_lstm = x_cnn.permute(0, 2, 1) 
        
        # 4. Operasikan ke LSTM
        # Pastikan lapisan LSTM Anda diinisialisasi dengan 'batch_first=True'
        out_lstm, _ = self.lstm(x_lstm)
        
        # 5. Ambil langkah terakhir (Last Time Step) untuk Prediksi Deformasi
        out = self.fc(out_lstm[:, -1, :])
        return out


class LSTMLayer(nn.Module):
    """LSTM layer dengan optional bidirectional."""
    
    def __init__(self, input_size: int, hidden_size: int, num_layers: int,
                 dropout: float = 0.2, bidirectional: bool = True):
        """
        Args:
            input_size (int): Input size
            hidden_size (int): Hidden size
            num_layers (int): Number of LSTM layers
            dropout (float): Dropout rate
            bidirectional (bool): Bidirectional LSTM
        """
        super().__init__()
        
        self.lstm = nn.LSTM(
            input_size=input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            dropout=dropout if num_layers > 1 else 0,
            bidirectional=bidirectional,
            batch_first=True
        )
        
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional
    
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass.
        
        Args:
            x (torch.Tensor): (batch, length, input_size)
        
        Returns:
            Tuple: (output, last_hidden)
        """
        output, (h_n, c_n) = self.lstm(x)
        
        if self.bidirectional:
            last_hidden = torch.cat([h_n[-2], h_n[-1]], dim=1)
        else:
            last_hidden = h_n[-1]
        
        return output, last_hidden


class CNNLSTMModel(nn.Module):
    """CNN-LSTM hybrid model untuk GNSS prediction."""
    
    def __init__(self, config: ModelConfig):
        """
        Initialize model.
        
        Args:
            config (ModelConfig): Model configuration
        """
        super().__init__()
        self.config = config
        
        # CNN branch
        if config.cnn_enabled:
            self.cnn = CNNLayer(
                in_channels=config.n_features,
                out_channels=config.cnn_filters[0],
                kernel_sizes=config.cnn_kernels,
                dropout=config.cnn_dropout
            )
            
            cnn_out_channels = len(config.cnn_kernels) * config.cnn_filters[0]
            cnn_out_length = config.input_length // 2
            cnn_out_size = cnn_out_channels * cnn_out_length
            
            logger.info(f"CNN output size: {cnn_out_size}")
        else:
            self.cnn = None
            cnn_out_size = config.input_length * config.n_features
        
        # LSTM branch
        lstm_input_size = config.n_features if self.cnn is None else cnn_out_channels
        lstm_hidden_size = config.lstm_hidden
        lstm_output_size = lstm_hidden_size * (2 if config.lstm_bidirectional else 1)
        
        self.lstm = LSTMLayer(
            input_size=lstm_input_size,
            hidden_size=lstm_hidden_size,
            num_layers=config.lstm_layers,
            dropout=config.lstm_dropout,
            bidirectional=config.lstm_bidirectional
        )
        
        # Dense layers
        dense_input = lstm_output_size
        self.dense_layers = nn.ModuleList()
        
        for hidden_units in config.dense_units:
            self.dense_layers.append(nn.Linear(dense_input, hidden_units))
            self.dense_layers.append(nn.ReLU())
            self.dense_layers.append(nn.Dropout(0.3))
            dense_input = hidden_units
        
        # Output layer
        self.output_layer = nn.Linear(dense_input, config.output_length * config.n_features)
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass.
        
        Args:
            x (torch.Tensor): (batch, length, n_features) atau (batch, n_features, length)
        
        Returns:
            torch.Tensor: (batch, output_length * n_features)
        """
        # CNN processing
        if self.cnn is not None:
            # CNN expects (batch, channels, length)
            if x.dim() == 3 and x.shape[1] > x.shape[2]:
                x_cnn = x  # Already in right format
            else:
                x_cnn = x.transpose(1, 2)
            
            x_cnn = self.cnn(x_cnn)
            x_lstm = x_cnn.reshape(x_cnn.shape[0], -1, self.config.n_features)
        else:
            x_lstm = x
        
        # LSTM processing
        _, lstm_out = self.lstm(x_lstm)
        
        # Dense layers
        out = lstm_out
        for layer in self.dense_layers:
            out = layer(out)
        
        # Output
        out = self.output_layer(out)
        return out


class ModelTrainer:
    """Trainer untuk CNN-LSTM model."""
    
    def __init__(self, model: CNNLSTMModel, config: ModelConfig):
        """
        Initialize trainer.
        
        Args:
            model (CNNLSTMModel): Model to train
            config (ModelConfig): Configuration
        """
        self.model = model
        self.config = config
        self.device = torch.device(config.device)
        
        self.model = self.model.to(self.device)
        
        # Optimizer
        self.optimizer = optim.Adam(
            self.model.parameters(),
            lr=config.learning_rate,
            weight_decay=config.weight_decay
        )
        
        # Loss function
        self.criterion = nn.MSELoss()
        
        logger.info(f"Trainer initialized on device: {self.device}")
        logger.info(f"Model parameters: {sum(p.numel() for p in self.model.parameters()):,}")
    
    def train_epoch(self, train_loader: DataLoader) -> float:
        """Train one epoch."""
        self.model.train()
        total_loss = 0.0
        n_batches = 0
        
        for x, y in train_loader:
            x = x.to(self.device)
            y = y.to(self.device)
            y = y.reshape(y.shape[0], -1)  # Flatten
            
            self.optimizer.zero_grad()
            pred = self.model(x)
            loss = self.criterion(pred, y)
            loss.backward()
            self.optimizer.step()
            
            total_loss += loss.item()
            n_batches += 1
        
        avg_loss = total_loss / n_batches
        return avg_loss
    
    def evaluate(self, val_loader: DataLoader) -> float:
        """Evaluate on validation set."""
        self.model.eval()
        total_loss = 0.0
        n_batches = 0
        
        with torch.no_grad():
            for x, y in val_loader:
                x = x.to(self.device)
                y = y.to(self.device)
                y = y.reshape(y.shape[0], -1)
                
                pred = self.model(x)
                loss = self.criterion(pred, y)
                
                total_loss += loss.item()
                n_batches += 1
        
        avg_loss = total_loss / n_batches
        return avg_loss
    
    def predict(self, x: np.ndarray) -> np.ndarray:
        """Make predictions."""
        self.model.eval()
        
        x_tensor = torch.FloatTensor(x).to(self.device)
        
        with torch.no_grad():
            pred = self.model(x_tensor)
        
        return pred.cpu().numpy()


def main_models():
    """Test models module."""
    config = ModelConfig()
    model = CNNLSTMModel(config)
    
    # Test forward pass
    x = torch.randn(32, config.input_length, config.n_features)
    y = model(x)
    
    logger.info(f"Input shape: {x.shape}")
    logger.info(f"Output shape: {y.shape}")
    logger.info(f"Expected output shape: (32, {config.output_length * config.n_features})")


if __name__ == "__main__":
    main_models()